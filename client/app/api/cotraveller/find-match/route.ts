...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('Finding CoTraveller matches:', JSON.stringify(matchRequest, null, 2));

        // Call backend API to find matches
        const backendResponse = await fetch(`${process.env.BACKEND_API_URL}/api/cotraveler/find-match`, {
            method: 'POST',
            headers: {
