    ...
    try {
      ...
      if (data.success && data.tripPlan) {
        ...
        const tripDetails: TripDetails = {
          ...
          experienceDatesStart: tripPlan.experienceDatesStart
            ? String(tripPlan.experienceDatesStart)
            : undefined,
          experienceDatesEnd: tripPlan.experienceDatesEnd
            ? String(tripPlan.experienceDatesEnd)
            : undefined,
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-8">
      ...
      <div className="flex gap-4">
        <Link href={`/plan/${tripId}/challenges`}>
          <Button variant="outline" className="flex items-center gap-2">
            <Trophy className="w-4 h-4" />
            View Challenges
          </Button>
        </Link>
        <Link href="/cotraveller/find">
          <Button variant="outline" className="flex items-center gap-2">
            ...
            <CardContent>
              <div className="space-y-2">
                <div>
                  <span className="font-medium">From:</span>{" "}
                  <span className="text-muted-foreground">
                    {formatDate(trip.experienceDatesStart, trip.dateInputType)}
                  </span>
                </div>
                {trip.experienceDatesEnd && (
                  <div>
                    <span className="font-medium">To:</span>{" "}
                    <span className="text-muted-foreground">
                      {formatDate(trip.experienceDatesEnd, trip.dateInputType)}
                    </span>
                  </div>
                )}
                {trip.duration && (
        ...
        trip.status === "failed") && (
          <div className="text-center py-10 border rounded-lg">
            ...
            <p className="text-muted-foreground">
              {trip.status === "pending" &&
                "Your experience itinerary is currently being planned. Please wait as we create your personalized experience plan."}
              {trip.status === "in-progress" &&
